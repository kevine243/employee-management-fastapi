...
from app.crud.user import create_user as crud_create_user, get_user_by_email, create_verification_token
...
async def create_user(
    ...
    verification = await create_verification_token(db, int(created_user.id))
    
    background_tasks.add_task(send_confirmation_email, created_user.email, verification.token)
...
async def confirm_email(token: str, db: AsyncSession = Depends(get_db)):
    ...
    await verify_user(db, verification.user_id, verification)  # ← user_id pas verification.user
